...
class BybitPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
        ...
        # Add wallet balance caching to reduce API calls
        self._wallet_balance_cache = {}
        self._wallet_balance_cache_timestamp = 0
        self._wallet_balance_cache_ttl = 30  # Cache TTL in seconds
        self._available_balance_cache = {}
        self._available_balance_cache_timestamp = {}
    ...
    def funding_fee_poll_interval(self) -> int:
        return 600  # Changed from 120 to 600 seconds (10 minutes) to reduce API calls
    ...
    async def _update_balances(self):
        ...
        # Check if we can use cached balance
        if (self._wallet_balance_cache and 
            current_time - self._wallet_balance_cache_timestamp < self._wallet_balance_cache_ttl):
            # Use cached balance
            unified_wallet_response = self._wallet_balance_cache
        else:
            ...
            # Update cache
            self._wallet_balance_cache = unified_wallet_response
            self._wallet_balance_cache_timestamp = current_time
    ...
    async def _fetch_available_balance(self, coin: str):
        ...
        # Check if we can use cached available balance for this coin
        if (coin in self._available_balance_cache and 
            coin in self._available_balance_cache_timestamp and
            current_time - self._available_balance_cache_timestamp[coin] < self._wallet_balance_cache_ttl):
            # Use cached balance
            return self._available_balance_cache[coin]
        ...
        available_balance = Decimal(available_balance_str)
        
        # Update cache
        self._available_balance_cache[coin] = available_balance
        self._available_balance_cache_timestamp[coin] = current_time
    ...
    async def _fetch_last_fee_payment(self, trading_pair: str) -> Tuple[int, Decimal, Decimal]:
        ...
        if not data:
            ...
            # Validate funding rate before using it
            try:
                funding_rate: Decimal = Decimal(str(last_data["funding"]))
                position_size: Decimal = Decimal(str(last_data["size"]))
                
                # Validate funding rate is within reasonable bounds (-0.1% to 0.1% per hour)
                if abs(funding_rate) > Decimal("0.001"):
                    self.logger().warning(
                        f"Unusual funding rate detected for {trading_pair}: {funding_rate}. "
                        f"Using previous value if available."
                    )
                    # Return invalid values to trigger fallback
                ...
                self.logger().error(
                    f"Error parsing funding data for {trading_pair}: {e}. Data: {last_data}"
                )
                return 0, Decimal("-1"), Decimal("-1")
