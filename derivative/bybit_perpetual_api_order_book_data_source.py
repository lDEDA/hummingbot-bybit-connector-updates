...
class BybitPerpetualAPIOrderBookDataSource(PerpetualAPIOrderBookDataSource):
    def __init__(
        ...
        self._max_reconnect_delay = 300  # 5 minutes maximum
    ...
    async def get_funding_info(self, trading_pair: str) -> FundingInfo:
        ...
        # Validate funding rate before creating FundingInfo
        try:
            funding_rate = Decimal(str(general_info["fundingRate"]))
            ...
            if abs(funding_rate) > Decimal("0.001"):
                self._connector.logger().warning(
                    ...
                    f"Clamping to reasonable bounds."
                )
                # Clamp to reasonable bounds instead of rejecting
                funding_rate = max(Decimal("-0.001"), min(funding_rate, Decimal("0.001")))
            ...
            self._connector.logger().error(
                f"Error parsing funding info for {trading_pair}: {e}. Data: {general_info}"
            )
            raise ValueError(f"Failed to parse funding info for {trading_pair}")
    ...
    async def _listen_for_subscriptions_on_url(self, url: str, trading_pairs: List[str]):
        ...
        while True:
            try:
                ...
                self._ws_connection_attempts[url] = 0
            except asyncio.CancelledError:
                raise
            except Exception as e:
                # Implement exponential backoff for reconnections
                attempts = self._ws_connection_attempts.get(url, 0)
                self._ws_connection_attempts[url] = attempts + 1
                
                # Calculate delay: 5 * 2^attempts seconds, capped at max_reconnect_delay
                delay = min(5 * (2 ** attempts), self._max_reconnect_delay)
                
                self.logger().exception(
                    f"Unexpected error occurred when listening to order book streams {url}. "
                    f"Attempt {attempts + 1}, retrying in {delay} seconds... Error: {str(e)}"
                )
