    ...
    async def listen_for_user_stream(self, output: asyncio.Queue):
        ...
        while True:
            try:
                ...
                delay = min(5 * (2 ** self._ws_connection_attempts), self._max_reconnect_delay)
                
                self.logger().exception(
                    f"Unexpected error while listening to user stream. "
                    f"Attempt {self._ws_connection_attempts}, retrying in {delay} seconds... Error: {str(e)}"
                )
